...
app.layout = html.Div(children=[
    html.H1('SpaceX Launch Records Dashboard v1',
...
def update_graph(site_dropdown):
    filtered_df=spacex_df
        if site_dropdown == 'ALL':
            fig = px.pie(data_frame= filtered_df, values='class', names=[Launch Site], title=f'Total Launches from All Sites')
            return fig
        else:
            specific_df=spacex_df.loc[spacex_df['Launch Site'] == site_dropdown]
            fig = px.pie(data_frame= specific_df, names=site_dropdown, values='class', title=f'Total Launch for {site_dropdown}')
            return fig
...
def get_scatter_plot(site_dropdown, payload_slider):
    if site_dropdown == 'ALL':
        ...
        figure = px.scatter(data_frame= data, y='class', x='Payload Mass (kg)', color='Booster Version',
