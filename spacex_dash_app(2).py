...
def update_graph('site-dropdown'):
        if site-dropdown == 'ALL':
            ...
            fig = px.pie(data_frame= filtered_df, values='class', 
            	names='site-dropdown', title=f'Total Launches from All Sites')
            return fig
        else:
            specific_df=spacex_df.loc[spacex_df['Launch Site'] == site-dropdown]
            fig = px.pie(data_frame= specific_df, names='site-dropdown',
                values='class', title=f'Total Launch for {site-dropdown}')
            return fig
